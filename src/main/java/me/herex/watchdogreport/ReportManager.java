...
public class ReportManager {
    ...
    public void openReportGUI(Player player, String reportedName) {
        ...
        // Player head (top center) - Hypixel style
        ...
        // Report reason items (Hypixel positions)
        ...
        menu.setItem(23, createMenuItem(Material.BANNER, ChatColor.GREEN + "Bad Skin", (short) 10)); // Green banner
        ...
        menu.setItem(30, createMenuItem(Material.MONSTER_EGG, ChatColor.GREEN + "Bad Pet name", (short) 98)); // Cow egg
    ...
    public void openConfirmationMenu(Player player, String reportedName, String reason) {
        ...
        ItemStack confirm = new ItemStack(Material.STAINED_CLAY, 1, (short) 13); // Green
        ItemMeta confirmMeta = confirm.getItemMeta();
        confirmMeta.setDisplayName(ChatColor.GREEN + "Submit Report");
        confirm.setItemMeta(confirmMeta);
        confirmMenu.setItem(11, confirm);

        ItemStack cancel = new ItemStack(Material.STAINED_CLAY, 1, (short) 14); // Red
    ...
    public void debugReportGUI(Player player, String reportedName) {
        plugin.getLogger().info("Debug: Opening report GUI for " + reportedName + " requested by " + player.getName());
        plugin.getLogger().info("Debug: Database connected: " + dbManager.isConnected());

        try {
            player.openInventory(createDebugMenu(reportedName));
            plugin.getLogger().info("Debug: GUI opened successfully");
        } catch (Exception e) {
            plugin.getLogger().severe("Debug: GUI error: " + e.getMessage());
            e.printStackTrace();
        }
    }

    private Inventory createDebugMenu(String reportedName) {
        Inventory menu = Bukkit.createInventory(null, 54, "Debug Report Menu");

        // Simple test item
        ItemStack test = new ItemStack(Material.PAPER);
        ItemMeta meta = test.getItemMeta();
        meta.setDisplayName("Test Item for " + reportedName);
        test.setItemMeta(meta);
        menu.setItem(22, test);
